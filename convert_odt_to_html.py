...
def extract_text_and_images(elem, zip_ref, images_output_dir):
    """
    從 ODT 元素中遞迴提取純文字與圖片。
    ...
    for child in elem:
        ...
        if tag == f"{{{ns['text']}}}s":
            ...
            text_content.append(extract_text_and_images(child, zip_ref, images_output_dir))
...
def process_block_element(elem, zip_ref, images_dir):
    ...
    if tag == f"{{{ns['text']}}}h":
         ...
         text = extract_text_and_images(elem, zip_ref, images_dir)
